...
Para realizar esta sintonização você deve manter pressionado o botão X (-) quando o dispositivo está no modo de Rádio FM. 
Isto irá armazenar todas as estações localizadas na memória interna do aparelho. Isso pode demorar alguns minutos. 

Então, para seleccionar entre as diferentes estações, basta clicar no botão Avanço (-) e Retrocesso (-), e vai mudar as estações que são armazenadas na memória interna. Para seleccionar manualmente uma estação segure o botão Avanço (-) e Retrocesso (-). 
