## Wireless settings

#### Bluetooth

Your Energy Tablet features Bluetooth technology to connect it to headphones, computers, tablets or any device that supports such communication protocol. You just need to go to Settings > Bluetooth and turn on the switch on the top part of the screen. Once enabled, the screen will start to show a list of available devices among which you will be able to select one to establish the connection.

![Imagen1](http://static.energysistem.com/images/manuals/42238/55912f8c38a42.jpg)
![Imagen2](http://static.energysistem.com/images/manuals/42238/55912f94323e5.jpg)
![Imagen3](http://static.energysistem.com/images/manuals/42238/55912fa819760.jpg)

1. Bluetooth
2. Turn the switch on.
3. Select the device you want to connect to.
4. Accept the pairing request.

### Wi-Fi

Your Energy Tablet Pro 3 has Wi-Fi technology. In order to connect your device to any wireless network, please go to Settings > Wi-Fi and turn on the switch located  on the top part of the screen. Once enabled, the screen will start to show a list of available devices among which you will be able to select one to establish the connection.

![Imagen1](http://static.energysistem.com/images/manuals/42238/55912f8c38a42.jpg)
![Imagen2](http://static.energysistem.com/images/manuals/42238/559130085b445.jpg)
![Imagen3](http://static.energysistem.com/images/manuals/42238/5591301d0ff15.jpg)

1. Wi-Fi.
2. Turn the switch on.
3. Select the wireless network you want to connect to.
4. Connect.

**Note:** Your Energy Tablet is compatible with the following security and encryption standards: WEP, WPA, WPA2, TKIP and AES. Once you have completed the authentication process successfully, your Energy Tablet will save the Wi-Fi access data and your device will be ready to access the Internet. The device will remember the network and will automatically connect to it whenever is available.
